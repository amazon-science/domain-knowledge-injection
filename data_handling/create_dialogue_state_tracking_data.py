...
# TODO: Test
# TODO: Add flag for whether DB access if required following user utterance?

# Note: This is a slightly modified variant of the KPRS construction script
# For DST, the auxiliary information supplied by the adapters is expected to inform DST prediction
# Here, too, fetched DB information is included, although it is not necessarily needed for the task

# SimpleTOD dialogue state representation:
#     a list of triplets recording values for slots in a particular domain: (domain, slot_name, value)
#     - should the model also generate belief states for domains that are not relevant? For now, no.

DOMAINS = ['restaurant', 'hotel', 'attraction', 'train', 'bus', 'hospital', 'police', 'taxi']


def _linearize_dialogue_state(turn_frames, verbalize=True):
    """ Linearizes the dialogue state for use as a generation target in model evaluation """

    # Note: If verbalize==False, only slot values are being looked at, similar to SimpleTOD

    # Only return a dialogue state target if active_intent is not None?
    dialogue_states = dict()
    dst_contents = dict()

    for domain_info in turn_frames:

        domain_seq, intent_seq, request_seq, slots_seq = None, None, None, None
        dst_content = dict()

        if domain_info['service'] != 'NONE':
            if verbalize:
                domain_seq = 'The user is asking about {} {}.'.format(
                    'an' if domain_info['service'].strip().lower() == 'attraction' else 'a', domain_info['service'])
            dst_content['domain'] = domain_info['service']

        if domain_info['state']['active_intent'] != 'NONE':
            if verbalize:
                verb, obj = domain_info['state']['active_intent'].split('_')
                intent_seq = 'The user is asking me to {} {} {}.'.format(
                    verb, 'an' if domain_info['service'].strip().lower() == 'attraction' else 'a', obj)
            dst_content['intent'] = domain_info['state']['active_intent']

        if len(domain_info['state']['requested_slots']) > 0:
            if verbalize:
                requests = []
                new_slots = []
                for slot in domain_info['state']['requested_slots']:
                    subj, obj = slot.split('-')
                    if obj == 'arriveby':
                        obj = 'time of arrival'
                    if obj == 'leaveat':
                        obj = 'time of departure'
                    if obj == 'trainid':
                        obj = 'number'
                    requests.append('about the {} of the {}'.format(obj, subj))
                    new_slots.append('{}_{}'.format(subj, obj))
                request_seq = 'The user has requested information {}.'.format(', '.join(requests))
                dst_content['requested_slots'] = new_slots

        if len(domain_info['state']['slot_values'].keys()) > 0:
            if verbalize:
                mentions = []
                new_slots = []
                for slot, value in domain_info['state']['slot_values'].items():
                    subj, obj = slot.split('-')
                    if obj not in ['parking', 'internet']:
                        if obj == 'arriveby':
                            obj = 'time of arrival'
                        if obj == 'leaveat':
                            obj = 'time of departure'
                        if obj == 'trainid':
                            obj = 'number'
                        mentions.append('the {} of the {} must be {}'.format(obj, subj, value[0]))
                    else:
                        if value[0] == 'yes':
                            mentions.append('the {} must have free {}'.format(subj, obj))
                        else:
                            mentions.append('the {} must not have free {}'.format(subj, obj))
                    new_slots.append('{}_{}_{}'.format(subj, obj, value[0]))
                slots_seq = 'The user has specified that {}.'.format(', '.join(mentions))
                dst_content['slot_values'] = new_slots
            else:
                slots = list()
                for slot_name, slot_value in domain_info['state']['slot_values'].items():
                    slots.append('{} {} {}'.format(domain_info['service'], slot_name.split('-')[-1], ' '.join(slot_value)))
                slots_seq = ', '.join(slots)
                dst_content['slot_values'] = ['{}_{}_{}'.format(k.split('-')[0], k.split('-')[1], v[0]) for k, v in
                                              domain_info['state']['slot_values'].items()]

        # if intent_seq is not None and domain_seq is not None:
        target_seq = ' '.join(seq for seq in [domain_seq, intent_seq, slots_seq, request_seq] if seq is not None)
        if len(target_seq.strip()) > 0:
            dialogue_states[domain_info['service']] = target_seq
            dst_contents[domain_info['service']] = dst_content

    if max([len(dialogue_states[k]) for k in dialogue_states.keys()] + [0]) > 0:
        return dialogue_states, dst_contents
    else:
        return None, None
        ...
        for v_id, v in enumerate(val):
            if v is None:
                ...
                if flag:
                    ...
                    if len(val) > 1 and v_id == 0:
                        if key == 'leaveat':
                            sql_query += r" {} ".format(connector) + ' {}'.format(
                                l_par) + key + " > " + r"'" + v2 + r"'"
                        elif key == 'arriveby':
                            sql_query += r" {} ".format(connector) + ' {}'.format(
                                l_par) + key + " < " + r"'" + v2 + r"'"
...
def collect_contexts(dialogue_files, dialogue_acts, database_dir, out_path, verbalize_ds):
    """ Collects all viable dialogue contexts to be used in the creation of the dialogue state tracking data. """
    ...
    for dia_id in dialogues_plus_acts.keys():
        ...
        for turn in dialogues_plus_acts[dia_id]['turns'].keys():
            ...
            if dialogues_plus_acts[dia_id]['turns'][turn]['speaker'] == 'USER':
                ...
                # Check if user turn requires DB access
                dia_acts = dialogues_plus_acts[dia_id]['turns'][turn]['dialog_act']
                requires_db = False
                for key in dia_acts.keys():
                    key = key.strip().lower()
                    if ('inform' in key or 'recommend' in key or 'request' in key or 'nooffer' in key) and 'booking' not in key:
                        requires_db = True
                        break
                ...
                updated_context.append((dialogues_plus_acts[dia_id]['turns'][turn], db_states))
                dialogue_states, dst_contents = \
                    _linearize_dialogue_state(dialogues_plus_acts[dia_id]['turns'][turn]['frames'], verbalize_ds)
                if dialogue_states is not None:
...
def create_samples(contexts_path):
    """ Creates DST samples form the collected contexts. """
    ...
    for dia_id in contexts.keys():
        ...
        for sample in contexts[dia_id]['contexts']:
            ...
            dst_sample = {k: v for k, v in sample.items()}
            ...
            dst_sample['sample_domains'] = list(context_domains)
            sample_set[dia_id]['samples'].append(dst_sample)
    ...
    for collection, path in [(single_domain_samples, single_domain_path_json),
        ...
        print('Saved {} contrastive samples to {}'.format(len(single_domain_samples_jsonl) if 'single' in path else
                                                          len(multi_domain_utterances_jsonl), path))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--verbalize_ds", action='store_true',
                        help="Whether to transform dialogue states into natural language passages")
    args = parser.parse_args()

    collect_contexts(args.dialogue_files, args.dialogue_acts, args.database_dir, args.out_path, args.verbalize_ds)
